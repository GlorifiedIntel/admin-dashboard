...
export default function Home() {
  ...
  if (session) {
    return (
      <>
      <div className="bg-[#DBAD6A] min-h-screen flex">
        <Nav />
        <div className="bg-[#FDFFFC] rounded-lg p-4">
        <p>Signed in as {session.user?.email}
        <button onClick={() => signOut()} className="bg-red-500 p-2 text-white rounded-lg px-4 mt-2">
          Sign out
        </button>
        </p>
        </div>
