    ...
    form.parse(req, async (err, fields, files) => {
      ...
      if (files.images) {
        ...
        for (const file of filesArray) {
          ...
          const uploadParams = {
            ...
            Key: `products/${Date.now()}-${file.originalFilename}`, // Unique file name
            Body: fileStream,
          ...
          try {
            ...
            imageUrls.push(uploadResult.Location); // Store the image URL returned from S3
          } catch (s3Error) {
            return reject(res.status(500).json({ error: "Error uploading to S3", details: s3Error.message }));
          }
        }
      }
      ...
      if (method === "PUT") {
        ...
        if (imageUrls.length > 0) {
          product.image = imageUrls; // Store array of image URLs from S3
        }
        await product.save();
